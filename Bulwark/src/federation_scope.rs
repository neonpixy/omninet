//! # Federation Scope — Data Boundary for Safety Operations
//!
//! From Constellation Art. 3 §3 — federation is a data boundary.
//!
//! When communities federate, they form a trust cluster. Safety data
//! (trust scores, reputation events, fraud indicators, health signals)
//! flows within a federation but not across federation boundaries.
//!
//! `FederationScope` is an optional filter applied to Bulwark queries.
//! When set, only data from visible communities is considered.
//! When empty (unrestricted), all communities are visible — this is the
//! default and preserves full backward compatibility.

use std::collections::HashSet;

use serde::{Deserialize, Serialize};

/// Optional federation scope for safety operations.
///
/// When set, trust and reputation data is scoped to federated
